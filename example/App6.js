...
xhr.onload = function() {
    let svg = this.responseXML.getElementsByTagName("path")
    let path = [];
    for (e of svg){
        path.push(e.attributes.d.textContent);
    };
    let svg_def = [
        {
            fill : "#EE0000",
    ...
    image = new SVG(svg_def);
    world.addElement(image);
    draw()
}
xhr.open("GET", "https://upload.wikimedia.org/wikipedia/commons/6/67/Firefox_Logo%2C_2017.svg");
xhr.send()
